...
void LCD_LayerInit(void)
{
  ...
  LTDC_Layer_InitStruct.LTDC_DefaultColorAlpha = 0; /* 完全透明 */
...
/**
 * @brief 设置 字体的颜色 及 字体的背景颜色（字模的非文字部分的颜色）（不是 背景层 的颜色）
 * @param TextColor: 字体颜色
 * @param BackColor: 字体的背景颜色（字模的非文字部分的颜色）（不是 背景层 的颜色）
 * @retval None
 */
...
/**
 * @brief 设置字体的背景颜色（字模的非文字部分的颜色）（不是 背景层 的颜色）
 * @param Color: 字体的背景颜色（字模的非文字部分的颜色）（不是 背景层 的颜色）
 * @retval None
 */
...
/**
 * @brief  设置 当前层的透明度，设置成功，该层所有像素包含该透明度
 * @param  transparency: 指定的透明度
 *         This parameter must range from 0x00 to 0xFF.
 *         0x00 -> 完全透明   0xFF -> 完全不透明
 * @retval None
 */
void LCD_SetTransparency(uint8_t transparency)
{
...
/**
 * @brief  把当前层 清屏显示 全黑 ，设置 文字颜色 为 当前文字颜色
 * @param  Color: the color of the background.
 * @retval None
 */
...
/**
 * @brief  显示一条 水平 或 竖直 直线（无法画任意角度直线）
 * @param Xpos: 直线起点的 x 坐标 , can be a value from 0 to 240.
 * @param Ypos: 直线起点的 y 坐标 , can be a value from 0 to 320.
 * @param Length: 直线的长度
...
void LCD_DrawLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length, uint8_t Direction)
{
  ...
  DMA2D_InitStruct.DMA2D_OutputBlue = Blue_Value;
  DMA2D_InitStruct.DMA2D_OutputRed = Red_Value;
  DMA2D_InitStruct.DMA2D_OutputAlpha = 0x0F; /* 未知怎么确定 */
  DMA2D_InitStruct.DMA2D_OutputMemoryAdd = Xaddress;
